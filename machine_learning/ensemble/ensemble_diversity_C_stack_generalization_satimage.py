...
X = data[:, :-1]
target = data[:,  -1]
t = np.zeros((X.shape[0], 7))
for s in range(X.shape[0]):
    t[s, target[s]-1]=1
t = np.delete(t, 5, axis=1)
t_sparse =  t
...
for cl in range(classifier_numbers):
    ...
    plt.legend(bbox_to_anchor=(0.05, 0.3), loc=2, fontsize=15, borderaxespad=0.)

for cm in range(combiner_numbers):
    plt.plot(combiner_accuracy_array[cm], c = colors[cl+cm], marker = 'o', linewidth = 2, label=combs[cm])
    plt.legend(bbox_to_anchor=(0.05, 0.3), loc=2, fontsize=12, borderaxespad=0.)
