...
def mlpPredict(x, WW):
    ...
    O = []
    for l in range(L):
        OO.append((np.ones((WW[l].shape[1],1))))
        O.append((np.zeros((WW[l].shape[0],1))))
...
def mlp(X, t, N, mu, maxEpoch, verbose=False):
    # Process 
    SN = X.shape[0]         
    L = len(N)-1
    sample=0
    ...
    Eepoch=np.zeros((1, SN))
    Earray=np.zeros((1, maxEpoch))
    
    for epoch in range(maxEpoch):
        
        for sample in range(SN):
        ...
        if(verbose==True):
            print('MLP epoch ={}   error={}'.format(epoch, Earray[0, epoch]))
