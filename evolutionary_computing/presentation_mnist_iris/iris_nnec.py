...
# @version  V2.0.0
# @date     29 May 2017
# @brief    Iris neural network evolutionary computing
...
Parent Selection ------------- 5 best and 3 worst
...
class evolutionary_computing():
    ...
    def evaluate_function(self, chromosomes):
        ...
        for chro in range(chromosomes.shape[0]):
            ...
            model = iris_perceptron_net.perceptron(self.generation[chro], n_classes=3)
            model.train_network()
    ...
    def initialize(self):
        ...
        self.generation = np.random.randint(3, 20, size=self.gene_num)
        for _ in range(self.population_size-1):
            self.generation = np.vstack([self.generation, np.random.randint(3, 20, size=self.gene_num)])
    ...
    def mutation(self):
            ...
            new_genes = np.random.randint(3, 20, size=self.offspring_num)
